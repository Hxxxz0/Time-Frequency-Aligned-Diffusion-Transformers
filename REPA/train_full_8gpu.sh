...
# 创建日志目录
mkdir -p logs
# 确保在 bash 中可用 conda 激活
source /data/wenshuo/miniconda3/etc/profile.d/conda.sh
conda activate repa
# 切到脚本所在目录，避免路径错误
cd "$(dirname "$0")"
# 启动训练，按照README标准格式
...
train.py \
  ...
  --encoder-depth=8 \
  --output-dir="exps" \
  --exp-name="dct-film-full-70k-4gpu_D_no_film_abl" \
  ...
  --enable-fid-eval \
  --fid-samples=5000 \
  > logs/train_4gpu_l2_multi_D_no_film_abl.log 2>&1 &

echo "训练已在后台启动！"
echo "查看训练日志: tail -f logs/train_4gpu_l2_multi_D.log"
echo "查看GPU使用: watch -n 1 nvidia-smi"
echo "查看tensorboard: tensorboard --logdir=exps/dct-film-full-70k-4gpu_dct"
